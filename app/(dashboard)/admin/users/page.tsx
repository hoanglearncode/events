...
import { Users, UserPlus, ShieldCheck, UserX, TrendingUp, Mail, Lock, Activity, Smartphone, MapPin, Edit, RotateCcw } from 'lucide-react';
...
const mockUsers: User[] = [
  {
    ...
    loginHistory: [
      { date: '2026-02-03 09:30', ip: '192.168.1.100', device: 'Chrome/Windows 10' },
      { date: '2026-02-02 14:20', ip: '192.168.1.100', device: 'Chrome/Windows 10' },
  ...
  },
  {
    ...
    loginHistory: [
      { date: '2026-02-03 11:00', ip: '203.162.4.190', device: 'Edge/Windows 11' },
      { date: '2026-02-02 09:00', ip: '203.162.4.190', device: 'Edge/Windows 11' }
    ]
  },
  {
    id: 'USR004',
    ...
    loginHistory: [
      { date: '2026-01-30 22:15', ip: '115.73.220.10', device: 'Chrome/Android' }
    ]
  },
  {
    id: 'USR005',
    ...
    loginHistory: [
      { date: '2026-02-02 18:30', ip: '14.231.15.88', device: 'Chrome/Windows 10' }
    ]
  },
  {
    id: 'USR006',
  ...
  },
  {
    ...
    loginHistory: [
      { date: '2026-02-03 08:45', ip: '192.168.10.50', device: 'Safari/iPad Pro' },
      { date: '2026-02-02 12:00', ip: '192.168.10.50', device: 'Safari/iPad Pro' }
    ]
  },
  {
    id: 'USR008',
    ...
    loginHistory: [
      { date: '2026-02-03 10:15', ip: '171.244.50.12', device: 'Chrome/Android' }
    ]
  }
];
...
const calculateStats = (users: User[]): UserStats => {
  ...
  const emailVerifiedRate = (users.filter(u => u.emailVerified).length / users.length * 100).toFixed(1);
  const suspendedBanned = users.filter(u => u.status === 'Suspended' || u.status === 'Banned').length;
...
const UserManagement: React.FC = () => {
  ...
  const getStatusBadge = (status: UserStatus) => {
    const variants: Record<UserStatus, { variant: 'default' | 'secondary' | 'destructive' | 'outline'; className: string }> = {
      'Active': { variant: 'default', className: 'bg-brand-success text-white hover:bg-brand-success/90' },
      'Suspended': { variant: 'secondary', className: 'bg-brand-warning text-white hover:bg-brand-warning/90' },
      'Banned': { variant: 'destructive', className: 'bg-brand-error text-white hover:bg-brand-error/90' },
      'Pending Verification': { variant: 'outline', className: 'border-brand-secondary text-brand-secondary' }
    };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Tổng quan và quản lý tài khoản người dùng</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {Object.entries(stats.roleCount).map(([role, count]) => (
                      <p key={role} className="text-xs text-muted-foreground">{role}: {count}</p>
                    ))}
                  </div>
                </div>
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Trong 7 ngày</p>
                  <p className="text-xs text-muted-foreground mt-1">{stats.newUsersLast30Days} trong 30 ngày</p>
                </div>
          ...
          <Card className="border-l-4 border-l-brand-secondary card-elevated">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium text-muted-foreground">Tỷ lệ xác thực email</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-3xl font-bold text-foreground">{stats.emailVerifiedRate}%</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {users.filter(u => u.emailVerified).length}/{stats.total} đã xác thực
                  </p>
                </div>
          ...
          <Card className="border-l-4 border-l-brand-error card-elevated">
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-3xl font-bold text-foreground">{stats.suspendedBanned}</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    {users.filter(u => u.status === 'Suspended').length} tạm khóa, {users.filter(u => u.status === 'Banned').length} cấm
                  </p>
                </div>
        ...
        <Card className="card-elevated">
          <CardHeader>
            ...
            <CardDescription>Quản lý và theo dõi thông tin chi tiết người dùng</CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <TableBody>
                  {filteredUsers.map((user) => (
                    <TableRow key={user.id} className="hover:bg-muted/50">
                      ...
                      <TableCell>
                        {user.emailVerified ? (
                          <Badge variant="outline" className="bg-brand-success/10 text-brand-success border-brand-success/30">
                      ...
                      <TableCell className="text-right">
                        <Dialog>
                          ...
                          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-card">
                            <DialogHeader>
                              <DialogTitle className="text-foreground">Chi tiết người dùng</DialogTitle>
                              <DialogDescription>
                              ...
                              <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-foreground mt-1">{selectedUser.createdAt}</p>
                                  </div>
                                  <div>
                                    <p className="text-sm font-medium text-muted-foreground">Đăng nhập cuối</p>
                                    <p className="text-sm text-foreground mt-1">{selectedUser.lastLogin}</p>
                                  </div>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div className="space-y-2">
                                    {selectedUser.loginHistory.map((login, idx) => (
                                      <div key={idx} className="bg-muted p-3 rounded-lg border border-border">
                                        <div className="flex items-start justify-between">
                                          <div className="space-y-1">
                                            <p className="text-sm font-medium text-foreground">{login.date}</p>
                                ...
                                <div className="flex gap-2 pt-4 border-t border-border">
                                  <Button variant="outline" className="flex-1 hover:bg-brand-primary/10 hover:text-brand-primary hover:border-brand-primary">
                                    <RotateCcw className="w-4 h-4 mr-2" />
                                    Đặt lại mật khẩu
                                  </Button>
                                  <Button variant="outline" className="flex-1 hover:bg-brand-secondary/10 hover:text-brand-secondary hover:border-brand-secondary">
                                    <Edit className="w-4 h-4 mr-2" />
                                    Chỉnh sửa
                                  </Button>
                                  {selectedUser.status === 'Active' ? (
                                    <Button variant="outline" className="flex-1 hover:bg-brand-error/10 hover:text-brand-error hover:border-brand-error">
