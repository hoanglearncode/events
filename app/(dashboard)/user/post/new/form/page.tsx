...
import React, { useState, useRef, useCallback, type FC, use, useEffect } from "react";
import {
  Type, List, CheckSquare, Star, Upload, Trash2, Plus,
  GripVertical, ChevronDown, Bold, Italic, Underline,
  AlignLeft, AlignCenter, AlignRight, Redo, Undo,
  Eye, EyeOff, Send, FileText, X, Check, Image, Link,
  Palette, AlignJustify, ListOrdered, ListIcon, Copy,
} from "lucide-react";
...
const QUESTION_TYPE_META: Record<QuestionType, { label: string; Icon: React.ComponentType<{ size?: number; color?: string }> }> = {
...
const RichTextEditor: FC<RichTextEditorProps> = ({ onChange, placeholder }) => {
  ...
  const execCmd = useCallback((cmd: string, val: string | undefined = undefined) => {
  ...
  const COLORS = ["#624a2b","#9c620f","#ab936c","#211d11","#10b981","#ef4444","#3b82f6","#8b5cf6"];

  const ToolBtn: FC<{ onClick: () => void; title: string; children: React.ReactNode }> = ({ onClick, title, children }) => (
    <button
  ...
  return (
    <div className="border border-border rounded-lg overflow-hidden bg-card">
      <div className="flex items-center gap-0.5 px-2 py-1.5 bg-muted border-b border-border flex-wrap">
        <ToolBtn onClick={() => execCmd("bold")} title="Bold (Ctrl+B)"><Bold size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("italic")} title="Italic (Ctrl+I)"><Italic size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("underline")} title="Underline (Ctrl+U)"><Underline size={14} /></ToolBtn>
        <Divider />
        <ToolBtn onClick={() => execCmd("justifyLeft")} title="Align Left"><AlignLeft size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("justifyCenter")} title="Align Center"><AlignCenter size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("justifyRight")} title="Align Right"><AlignRight size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("justifyFull")} title="Justify"><AlignJustify size={14} /></ToolBtn>
        <Divider />
        <ToolBtn onClick={() => execCmd("insertUnorderedList")} title="Bullet List"><ListIcon size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("insertOrderedList")} title="Numbered List"><ListOrdered size={14} /></ToolBtn>
        <Divider />
        <div className="relative">
          <button
            type="button" onClick={() => setColorOpen(!colorOpen)}
          ...
          >
            <Palette size={14} /><ChevronDown size={10} className={`transition-transform ${colorOpen ? "rotate-180" : ""}`} />
          </button>
          {colorOpen && (
            <div className="absolute top-full left-0 mt-1 z-50 bg-card border border-border rounded-lg p-2 shadow-lg grid grid-cols-4 gap-1">
              {COLORS.map(c => (
                <button
                  key={c} type="button"
                  onClick={() => { execCmd("foreColor", c); setColorOpen(false); }}
        ...
        <Divider />
        <ToolBtn onClick={() => execCmd("undo")} title="Undo"><Undo size={14} /></ToolBtn>
        <ToolBtn onClick={() => execCmd("redo")} title="Redo"><Redo size={14} /></ToolBtn>
        ...
        <div className="ml-auto">
          ...
          >
            {showSource ? <><EyeOff size={12} /> Editor</> : <><Eye size={12} /> HTML</>}
          </button>
        </div>
      </div>
...
const FilePreview: FC<FilePreviewProps> = ({ file, onRemove }) => (
  <div className="flex items-center gap-3 bg-muted border border-border rounded-lg px-3 py-2">
    {file.preview ? (
      <img src={file.preview} alt="" className="w-9 h-9 rounded-md object-cover" />
    ) : (
...
const QuestionEditor: FC<QuestionEditorProps> = ({ question, index, onUpdate, onRemove, onDuplicate }) => {
  const [typeOpen, setTypeOpen] = useState(false);
  const meta = QUESTION_TYPE_META[question.type];
  const hasOptions = OPTION_TYPES.includes(question.type);

  const addOption = () =>
    onUpdate({ ...question, options: [...question.options, { id: uid(), text: "" }] });

  const updateOption = (id: string, text: string) =>
    onUpdate({ ...question, options: question.options.map(o => (o.id === id ? { ...o, text } : o)) });
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-2 mb-3.5">
        ...
        <span className="text-xs text-muted-foreground flex-1">Câu hỏi {index + 1}</span>
        <button type="button" onClick={onDuplicate} className="p-1.5 text-muted-foreground hover:text-accent transition-colors" title="Nhân đôi">
          <Copy size={15} />
        </button>
        <button type="button" onClick={onRemove} className="p-1.5 text-muted-foreground hover:text-destructive transition-colors" title="Xóa">
          <Trash2 size={15} />
        </button>
      </div>
      ...
      <div className="relative mt-2.5">
        <button
          type="button" onClick={() => setTypeOpen(!typeOpen)}
        ...
        >
          ...
          <ChevronDown size={13} className={`ml-auto transition-transform ${typeOpen ? "rotate-180" : ""}`} />
        </button>
        ...
        {typeOpen && (
          <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-card border border-border rounded-lg shadow-lg overflow-hidden max-h-64 overflow-y-auto">
            {(Object.entries(QUESTION_TYPE_META) as [QuestionType, typeof QUESTION_TYPE_META[QuestionType]][]).map(([type, m]) => (
              <button
                ...
                onClick={() => {
                  ...
                  onUpdate({
                    ...
                    type,
                    options: needsOpts
                      ? (question.options.length ? question.options : [{ id: uid(), text: "Tùy chọn 1" }, { id: uid(), text: "Tùy chọn 2" }])
                      : [],
                  });
                  setTypeOpen(false);
                }}
              ...
              >
                <m.Icon size={14} color={question.type === type ? "var(--color-accent)" : "var(--color-muted-foreground)"} />
                {m.label}
              </button>
            ))}
          </div>
        ...
        <div className="mt-3 space-y-1.5">
          {question.options.map((opt, i) => (
            <div key={opt.id} className="flex items-center gap-2">
              <div className={`w-4.5 h-4.5 flex-shrink-0 border-2 border-primary ${question.type === "checkbox" ? "rounded" : "rounded-full"}`} />
              <input
              ...
              />
              <button type="button" onClick={() => removeOption(opt.id)} className="text-muted-foreground hover:text-destructive transition-colors p-0.5">
                <X size={13} />
              </button>
            </div>
          ))}
      ...
      <div className="flex items-center gap-2.5 mt-4">
        ...
        >
          <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${question.required ? "left-4.5" : "left-0.5"}`} />
        </button>
...
const FormPreview: FC<FormPreviewProps> = ({ questions, formTitle, formDesc }) => {
  ...
  const handleFileUpload = (qId: string, files: FileList, imageOnly: boolean) => {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-14 px-4">
        ...
        <h2 className="text-2xl font-semibold text-foreground mb-2">Đã gửi thành công!</h2>
        <p className="text-sm text-muted-foreground mb-8">Phản hồi của bạn đã được ghi nhận. Cảm ơn bạn!</p>
        <button
          type="button"
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 space-y-3.5">
      ...
      <div className="bg-card border-t-4 border-primary border border-border rounded-xl px-6 py-7 shadow-sm">
        <h1 className="text-xl font-semibold text-foreground mb-1.5">{formTitle || "Tiêu đề form"}</h1>
        {formDesc && <p className="text-sm text-muted-foreground leading-relaxed">{formDesc}</p>}
      </div>

      {questions.length === 0 && (
        <div className="text-center py-12 text-sm text-muted-foreground">Chưa có câu hỏi nào.</div>
      )}
        ...
        return (
          ...
          >
            ...
            <div className="flex items-start gap-2.5 mb-3">
              ...
              <div>
                <span className="text-sm font-medium text-foreground">{q.text || `Câu hỏi ${idx + 1}`}</span>
            ...
            {q.type === "textarea" && (
              <textarea
                rows={4} placeholder="Nhập câu trả lời dài..."
                value={(answers[q.id] as string) || ""}
                onChange={e => setAns(q.id, e.target.value)}
            ...
            {/* ── Rich text ── */}
            {q.type === "richtext" && (
              <RichTextEditor value={(answers[q.id] as string) || ""} onChange={v => setAns(q.id, v)} placeholder="Nhập nội dung..." />
            )}
            ...
            {q.type === "radio" && (
              <div className="space-y-1">
                {q.options.map(opt => (
                  ...
                  >
                    <input type="radio" checked={(answers[q.id] as string) === opt.id} onChange={() => setAns(q.id, opt.id)} className="sr-only" />
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors
                      ${(answers[q.id] as string) === opt.id ? "border-primary" : "border-border"}`}>
                      {(answers[q.id] as string) === opt.id && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
                    </div>
              ...
              <div className="space-y-1">
                {q.options.map(opt => {
                  ...
                  return (
                    ...
                    >
                      <input type="checkbox" checked={checked} onChange={() => toggleCheckbox(q.id, opt.id)} className="sr-only" />
              ...
              <div className="flex items-center gap-1">
                {[1, 2, 3, 4, 5].map(n => (
                  ...
                  >
                    <Star
                      size={28}
                      fill={(answers[q.id] as number) >= n ? "var(--color-accent)" : "none"}
                      color={(answers[q.id] as number) >= n ? "var(--color-accent)" : "var(--color-border)"}
                    />
                  </button>
                ))}
                {answers[q.id] && <span className="text-xs text-muted-foreground ml-2">{answers[q.id]}/5</span>}
              </div>
            )}
              ...
              <div>
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-border rounded-lg px-4 py-6 cursor-pointer hover:border-primary hover:bg-muted/40 transition-colors">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {q.type === "image" ? "Drag & drop hình ảnh hoặc click để chọn" : "Drag & drop tệp hoặc click để chọn"}
                  </span>
                  <span className="text-xs text-muted-foreground/60 mt-1">
                    {q.type === "image" ? "PNG, JPG, GIF" : "Tất cả các loại tệp"}
                  </span>
                  <input
                    type="file" multiple
                    accept={q.type === "image" ? "image/*" : undefined}
                    onChange={e => handleFileUpload(q.id, e.target.files!, q.type === "image")}
                    className="sr-only"
                  />
                </label>
                {((answers[q.id] as UploadedFile[]) || []).length > 0 && (
                  <div className="mt-2.5 space-y-1.5">
                    {((answers[q.id] as UploadedFile[]) || []).map((f, i) => (
                      <FilePreview key={i} file={f} onRemove={() => removeFile(q.id, i)} />
                    ))}
                  </div>
                )}
              </div>
            ...
            {/* Error */}
            {errors[q.id] && (
              <p className="text-xs text-destructive mt-1.5 flex items-center gap-1">⚠ Trường này bắt buộc</p>
            )}
          </div>
        );
      })}
...
const DEFAULT_QUESTIONS: Question[] = [
  { id: "q1", type: "text",     text: "Tên đầy đủ của bạn?",                          required: true,  options: [] },
  { id: "q2", type: "radio",    text: "Bạn sử dụng nền tảng này như thế nào?",        required: true,  options: [{ id: "o1", text: "Công việc" }, { id: "o2", text: "Học thuật" }, { id: "o3", text: "Giải trí" }, { id: "o4", text: "Khác" }] },
  { id: "q3", type: "checkbox", text: "Chọn các tính năng bạn thích nhất:",           required: false, options: [{ id: "c1", text: "Giao diện đẹp" }, { id: "c2", text: "Tốc độ nhanh" }, { id: "c3", text: "Nhiều tính năng" }, { id: "c4", text: "Dễ sử dụng" }] },
  { id: "q4", type: "rating",   text: "Đánh giá trải nghiệm tổng cộng của bạn:",     required: false, options: [] },
  { id: "q5", type: "richtext", text: "Chia sẻ phản hồi chi tiết của bạn:",           required: false, options: [] },
  { id: "q6", type: "file",     text: "Upload tài liệu đính kèm (nếu có):",           required: false, options: [] },
];
...
const GoogleFormClone: FC = () => {
  ...
  const addQuestion = () =>
    setQuestions(prev => [...prev, { id: uid(), type: "text", text: "", required: false, options: [] }]);
  ...
  const duplicateQuestion = (id: string) => {
    ...
    if (q) setQuestions(prev => [...prev, { ...q, id: uid(), options: q.options.map(o => ({ ...o, id: uid() })) }]);
  };
      ...
      <main className={`max-w-5xl mx-auto px-4 py-7`}>
        {mode === "builder" ? (
          <>
            ...
            <div className="mt-6">
              <p className="text-xs uppercase tracking-wide text-muted-foreground mb-2 font-medium">Thêm nhanh theo loại:</p>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setQuestions(prev => [...prev, {
                        id: uid(), type, text: "", required: false,
                        options: needsOpts ? [{ id: uid(), text: "Tùy chọn 1" }, { id: uid(), text: "Tùy chọn 2" }] : [],
                      }]);
                    }}
          ...
          <FormPreview questions={questions} formTitle={formTitle} formDesc={formDesc} />
        )}
      </main>
    </div>
  );
